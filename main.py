...
from corpora.comparative_adjectives import (
    COMPARATIVE_ADJECTIVES as comparative_adjectives,
)
...
def verb_ing(verb):
    ...
    if len(verb) >= 3 and not verb.endswith(
        ("y", "w", "x")
    ):  # Exclude common non-doublers
...
def verb_ed(verb):
    ...
    if (
        len(verb) >= 3
        and last_letter not in vowels
        and verb[-2].lower() in vowels
        and verb[-3].lower() not in vowels
    ):
...
def makeReply(user_acct, text, in_reply_to_id):
    ...
    if len(text.strip()) < 2:
        ...
        status = f'@{user_acct} {text_object["sentence"]}'
